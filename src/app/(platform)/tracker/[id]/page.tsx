...
import { deleteExpenseAction, getExpenseAction, updateExpenseAction } from "@/app/(platform)/tracker/actions";
...
export default function Page(props: RouteProps) {
  ...
  const handleSave = async (data: FormData) => {
    ...
    try {
      await updateExpenseAction(id, expense);
    ...
    <Box className={"grid h-full place-items-center"}>
      <Paper className={"w-96 p-8"} variant={"outlined"}>
        <Box component={"form"} className={"flex flex-col gap-4"} action={handleSave}>
          <Typography className={"text-center text-2xl font-bold"}>Edit Expense</Typography>
